...
class TokenMath:
    ...
    @staticmethod
    def exp_moving_average(data, window: float):
        """
    ...
    @staticmethod
    def unusual_value(distribution, test_value: float):
        """
    ...
    def parse_token_data(token_history):
        ...
        # 2 hour change, 6 hour change, 12 hour change, 24 hour change, 48 hour change
        CHANGE_PERIODS = [2, 6, 12, 24, 48]
        moving_price_changes = [(TokenMath.exp_moving_average(price_data, WINDOW)[
                                 period:] / (TokenMath.exp_moving_average(price_data, WINDOW)[:-period] + EPSILON))[-1] for period in CHANGE_PERIODS]
        ...
        for moving_price_change, reversed_change_period in zip(moving_price_changes, CHANGE_PERIODS[::-1]):
            partial_price = moving_price_change * \
                np.math.exp(np.math.sqrt(reversed_change_period)) / exp_sum
        ...
        price_changes = [((price_data[period:] / (price_data[:-period] + EPSILON))
                          [-1] - 1) * 100 for period in CHANGE_PERIODS]
