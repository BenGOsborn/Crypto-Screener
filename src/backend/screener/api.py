...
class API:
    ...
    def get_token_info(self, num_tokens: int):
        """
        ...
        for page_number in range(num_pages):
            ...
            if request.ok:
                ...
                token_info += [{'id': data['id'], 'symbol': data['symbol'], 'name': data['name'],
                                'url': f"https://www.coingecko.com/en/coins/{data['id']}", 'image': data['image']} for data in form_json]
        ...
        print(
            f"Got token info for {len(token_info)} tokens. Missing: {num_tokens - len(token_info)}")

        return token_info

    def get_token_history(self, token_id: str, days: int = 7):
        """ 
        ...
        if request.ok:
            form_json = request.json()
            token_data = np.array(
                [form_json['prices'], form_json['total_volumes']])

            # Remove the column that contains the times the events occured
            return token_data[:, :, 1]
